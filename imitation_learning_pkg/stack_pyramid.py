...
# 현재 스크립트 위치(imitation_learning_pkg)에서 두 단계 위인 src 폴더의 절대 경로를 구함
...
@register_env
class StackPyramid(ManipulationEnv):
    """
    ...
    def staged_rewards(self):
        ...
        """
        #                   reach 관련 보상 설정 합은 0.5가 되도록.   +    기준은 B가 되도록. B는 초록색
        ...
        dist_C = min(
            [
                np.linalg.norm(self.sim.data.site_xpos[self.robots[0].eef_site_id[arm]] - cubeC_pos)
                for arm in self.robots[0].arms
            ]
        )

        r_reach_A = (1 - np.tanh(10.0 * dist_A)) * 0.125
        r_reach_C = (1 - np.tanh(10.0 * dist_C)) * 0.125
        ...
        if grasping_cubeC:
            r_reach_C += 0.125
        r_reach = r_reach_A + r_reach_C

        #               리프팅 관련 보상 설정 합은 1.5가 되도록.   <=   리프트 1, 얼라인 0.5
        ...
        cubeC_lifted = cubeC_height > table_height + 0.04
        r_lift_A = 0.5 if cubeA_lifted else 0.0
        r_lift_C = 0.5 if cubeC_lifted else 0.0
        r_lift = r_lift_A + r_lift_C

        # 얼라인 성공했는지.
        if cubeC_lifted:
            horiz_dist_BC = np.linalg.norm(np.array(cubeC_pos[:2]) - np.array(cubeB_pos[:2]))
            r_lift += 0.25 * (1 - np.tanh(horiz_dist_BC))
        if cubeA_lifted:
            pos_BC_mid = (np.array(cubeB_pos) + np.array(cubeC_pos)) / 2
            horiz_dist_A_BC = np.linalg.norm(np.array(cubeA_pos[:2]) - np.array(pos_BC_mid[:2]))
            r_lift += 0.25 * (1 - np.tanh(horiz_dist_A_BC))

        # A랑 B, A랑 C 모두 컨택됐는지 확인. + 높이도 제대로 올라간거까지 체크 추가.
        # stacking is successful when the block is lifted and the gripper is not holding the object
        r_stack = 0
        cubeA_touching_cubeB = self.check_contact(self.cubeA, self.cubeB)
        cubeA_touching_cubeC = self.check_contact(self.cubeA, self.cubeC)
        a_is_higher = cubeA_pos[2] > cubeB_pos[2] + 0.02 and cubeA_pos[2] > cubeC_pos[2] + 0.02
        if not grasping_cubeA and not grasping_cubeC and r_lift > 0 and cubeA_touching_cubeB and cubeA_touching_cubeC and a_is_higher:
            r_stack = 2.0
    ...
    def _load_model(self):
        ...
        current_dir = os.path.dirname(os.path.abspath(__file__))
        xml_path = os.path.join(current_dir, "assets", "target_cube.xml")

        try:
            tree = ET.parse(xml_path)
            root = tree.getroot()
            print("찾음")
            for site in root.find("worldbody").findall("site"):
